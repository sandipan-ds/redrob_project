...
**Result:** ✅ 20/20 passed (54.85s — dominated by model inference in semantic tests)
...
The 20 tests are split across four groups.
...
pytest reported `20 passed in 54.85s` with exit code `0`. The runtime is long because the three semantic tests load the sentence-transformers model and run inference on short text pairs — this is expected and only happens at test time, not at ranking runtime. All 20 tests showed `PASSED` individually with no warnings, skips, or expected failures.
